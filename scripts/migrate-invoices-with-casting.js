  ...
  try {
    ...
    backupDb.all("SELECT * FROM invoices", async (err, invoices) => {
      ...
      for (const invoice of invoices) {
        try {
          // Use raw SQL to insert with all required fields and proper enum casting
          const insertQuery = `
            INSERT INTO "public"."invoices" (
              ...
              $1, $2, $3, $4, $5, $6, $7::"public"."InvoiceStatus", $8, $9, $10, $11, $12
            )
          `;
...
async function migrateInvoiceItemsFromBackup() {
  ...
  backupDb.all("SELECT * FROM invoice_items", async (err, items) => {
    ...
    for (const item of items) {
      try {
        // Use raw SQL to insert invoice items with proper enum casting
        const insertQuery = `
          INSERT INTO "public"."invoice_items" (
            "id", "invoice_id", "description", "quantity", "rate", 
            "total", "agency_commission", "business_area", "created_at"
          ) VALUES (
            $1, $2, $3, $4, $5, $6, $7, $8::"public"."BusinessArea", $9
          )
        `;
